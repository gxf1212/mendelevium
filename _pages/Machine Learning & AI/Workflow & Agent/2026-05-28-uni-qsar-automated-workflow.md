---
title: "Uni-QSAR：自动化分子性质预测工作流的新框架"
date: "2026-05-28"
...
> 近年来，基于**深度学习（DL）**的定量构效关系（QSAR）模型在药物发现的性质预测任务中展现出了超越传统方法的性能。然而，大多数基于DL的QSAR模型受限于有限的标注数据，且对**模型尺度**和**超参数**非常敏感。在本文中，我们提出了**Uni-QSAR**，这是一个用于分子性质预测任务的强大**Auto-ML工具**。Uni-QSAR通过预训练模型，将**1D序列标记、2D拓扑图和3D构象**的分子表示学习（MRL）与大规模无标签数据的丰富表示相结合。在设计的并行工作流下，**无需任何人工微调或模型选择**，Uni-QSAR在Therapeutic Data Commons（TDC）基准的22个任务中，有**21个达到了SOTA表现**，平均性能提升达**6.09**%。此外，我们还展示了Uni-QSAR在药物发现领域的实际应用价值。

### 核心亮点

- **多表示集成**：同时利用1D SMILES序列、2D分子图、3D构象信息，通过预训练模型提取丰富特征
- **自动化程度高**：减少人工超参数调节和模型选择，实现端到端自动化建模
- **并行加速**：基于Dflow的分布式工作流，在实验中实现约2倍训练加速，按需分配计算资源
- **CNS应用验证**：在CNS药物候选富集案例中，将10亿级化合物库逐步筛到130万个CNS-like ligands
...
> 如何把**1D、2D、3D表示和自动化模型选择**放进同一套工作流，且打破严重依赖手工微调的窘境，是本文要解决的核心问题。
...
- **1D序列表示**：处理SMILES字符串编码的分子结构。本文使用K-BERT等预训练模型，通过原子预测、分子特征预测和对比学习等预训练任务，学习分子的序列模式。
- **2D拓扑图表示**：构建原子和键的图结构。利用图神经网络的节点/边上下文预测、图级motif预测、对比学习等策略捕捉拓扑特征。图结构能够明确编码分子的拓扑连接信息，克服字符串表示无法编码重要拓扑信息的局限。
- **3D构象表示**：通过原子几何坐标和SE(3)等变Transformer学习空间表示，利用位置去噪任务和掩码原子预测任务捕获构象特征。

> **消融实验的启示**：加入Uni-Mol这类**纯3D预训练模型**能显著提升Uni-QSAR的整体平均性能。但原文也客观提醒：3D MRL并非所有QSAR任务中的“万金油”，例如hERG和pKa等任务依然可能更依赖局部的官能团或化学特征。
...
- **Input Processing（输入处理）**：执行异常值过滤、缺失值处理和目标变量归一化；回归任务中根据偏度选择标准化、Box-Cox或Yeo-Johnson变换。
- **FeatureHub（特征中心）**：集成Morgan fingerprints、分子描述符、1D tokens、2D graphs和3D conformers等多种表示，并适配相应预训练模型。
- **ModelHub（模型中心）**：包含SVM、LR、GBDT、ExtraTrees等传统机器学习模型，以及Uni-Mol、BERT、GNN、EGNN等预训练模型或神经网络模型。
- **Task（任务管理）**：负责模型选择、stacking、日志、缓存和监控，并与底层workflow基础设施配合完成任务调度。
...
> Uni-QSAR并不是简单地把所有特征糅合塞进一个统一的“超级大模型”中，而是设计了一套**自动化集成学习（Auto Stacking）**系统。就像一个精明的“包工头”，让适应不同任务的“特征+模型”分别干活后再协同出结论。

- **多组合并行竞争**：系统会自动构建众多**基学习器**（Base Learners），每个基学习器是一个特定的【表示方法+匹配模型】组合。
  - 例如，处理局部化学环境时采用**“2D拓扑图+GNN”**，预测序列规律时采用**“1D SMILES+BERT”**，或在特定任务中使用传统的**“分子指纹+GBDT”**。
  - 这些组合会在数据上并行训练，并通过交叉验证的方式自动淘汰评估得分较低的冗余组合。
- **两级堆叠（Stacking）集成**：对于筛选出的优胜模型组合，工作流不会只取其一。
  - **第一层（Level 1）**将所有基学习器的预测输出作为新的输入特征，喂给下一层的主分类/回归器（Meta Learners，如支持向量机、多层感知机或逻辑回归）去重新拟合。
  - **第二层（Level 2）**则进一步加入简单的平均操作（AVG），以提升整体Stacking的泛化能力和抗过拟合能力。
- **自适应任务纠正**：
  - 对于**类别不平衡**的任务，Uni-QSAR先利用训练集中类别比例的简单阈值规则识别不平衡现象，再分别自动尝试Focal Loss和GHM Loss进行损失惩罚。
  - 在**回归任务**中，系统还能根据目标分布的偏度数据，自动决定是否应用非线性的目标归一化（Target Normalization）。
...
- **21/22任务达到SOTA表现**：在22个ADME/T任务中的21个任务上达到论文所称SOTA表现，平均性能提升**6.09**%
- **任务覆盖全面**：包含18个ADME任务和4个Tox任务，全面覆盖药物发现的关键性质预测需求
- **评估指标多样**：回归任务使用MAE（平均绝对误差）和Spearman相关系数，分类任务使用AUROC（ROC曲线下面积）和AUPRC（精确率-召回率曲线下面积）
...
- **不能简单理解为单一表示胜出**：原文没有系统给出“纯1D、纯2D、纯3D”三者的完整对比，而是通过去除Uni-Mol、去除stacking、去除normalization来验证模块贡献。
...
- **第一步：类药性过滤**，对10亿级可购买数据库应用Lipinski五法则和Veber规则，过滤不可成药分子。
- **第二步：相似性粗筛**，基于fingerprints bit vectors进行相似性搜索，得到1300万个ligand DB候选。
- **第三步：QSAR精筛**，使用Uni-QSAR对候选分子进行排序选择，最终得到130万个CNS-like ligands，用于后续先导优化、ADMET预测和re-docking等流程。
...
- **预训练模型成本**：多表示集成会引入更复杂的特征提取和模型训练，尤其是使用深度学习和3D预训练表示时，需要更好的算力调度。
- **3D信息不总是最优**：原文明确指出3D MRL并不总是在各种QSAR任务中最佳，hERG、pKa等任务可能更依赖关键官能团或局部化学环境。
- **论文未展开可解释性**：本文重点在Auto-ML工作流和性能验证，没有系统讨论模型解释性，实际药物化学优化时仍需要额外解释工具。
- **适用范围仍需扩展验证**：论文主要验证ADME/T和CNS候选富集场景；作者在Future Work中提到会扩展到小分子、大分子RNA、材料和化学品等更多属性预测场景。
...
Uni-QSAR的价值在于同时回应了传统QSAR建模的三个痛点：**标注数据稀缺**、**模型选择困难**和**资源利用低效**。通过多表示预训练模型利用无标签数据，通过AutoML减少手动调参，通过Dflow并行工作流优化资源配置，这三点共同支撑了论文中的性能结果。

### 对药物发现的影响

对于药物发现实践而言，Uni-QSAR的意义在于让研究人员更方便地进行分子性质预测和候选分子排序，从而辅助先导化合物发现和优化。它不替代实验验证，但可以把**需要实验优先检查的候选空间**缩小到更可管理的范围。
