...
> 数据整理最怕黑箱：**MolJam把标准化到打分的每一步都做成可复现、可质问的环节**，任何一条记录为何被改、为何被删，都能说清楚。
...
> 五个维度、十二个指标，最后汇成一个数字，而数字真正的用处，是**能顺着分数反查到具体哪条记录出了问题**——而不是笼统地“感觉这个数据集不太行”。

#### 打分函数

两个核心函数，12个指标都通过它们把原始缺陷率/缺陷数换算成0–10分。

**比例型函数 $Q(r)$**：以缺陷率 $r \in [0, 1]$ 为输入，分三段扣分。低缺陷率（$r < 0.1$）线性扣分；$0.1 \le r < 0.5$ 换二次多项式加速惩罚；$r \ge 0.5$ 走指数函数进一步加重。三段之间保持 $C^1$ 连续，**错得越多，罚得越重，而且越来越快**：

$$
Q(r) =
\begin{cases}
10(1 - r) \quad r < 0.1 \\
a(r - 0.1)^2 + b(r - 0.1) + c \quad 0.1 \le r < 0.5 \\
2.5 \, \exp\bigl(k(r - 0.5)\bigr) \quad r \ge 0.5
\end{cases}
$$

其中系数 $a, b, c, k$ 由连续性条件唯一确定（详见Supplementary §S1）。

**计数型函数 $C(c, N)$**：同时看缺陷绝对数量 $c$ 和数据集规模 $N$，绝对数量取对数惩罚、缺陷比例走线性惩罚，并设上限 $\mu$ 封顶：

$$
C(c, N) = 10 \left[ 1 - \min\!\left(\mu,\; \beta \log_{10}(c+1) + \beta\lambda \dfrac{c}{N}\right) \right]
$$

参数 $(\beta, \lambda, \mu)$ 分三档：低重要性用 $(0.05, 20, 0.5)$，中等重要性（表示一致性、结构重复）用 $(0.1, 50, 0.7)$，高重要性（SMILES有效性、标签矛盾）用 $(0.2, 100, 0.9)$。**小数据集里的一两个硬伤照样扣分，大数据集里的严重问题也不会因为分母大而变轻**。

#### 聚合与低分惩罚

各指标先归一化进所属维度，每维度最高贡献20分，五维加总得到0–100的原始分 $S_{\text{raw}}$。再用改进的Cannikin Law叠加惩罚：

- 单个指标得分 $s_i < 6$ 时开始扣分：$s_i \in [2, 4)$ 扣0.5，$[4, 6)$ 扣0.1，$[0, 2)$ 扣1.5
- 令 $\rho$ 为低于6分的指标占比；当 $\rho > 0.3$ 时，额外加罚 $1.2(\rho - 0.3)$

$$
\Pi = \min\!\left(\; \sum_i \pi_i \;+\; 1.2 \max\bigl(0,\, \rho - 0.3\bigr) \;,\; 30 \right)
$$

最终总分减去惩罚并截断到0：

$$
S_{\text{final}} = \max\bigl(0,\; S_{\text{raw}} - \Pi\bigr)
$$

**短板效应在这里变成具体数值**：一个极低分指标（$s_i < 2$）一个就扣1.5分；若多数指标都偏低，额外惩罚让总分被进一步压低，最高扣30分。这解释了为什么某些数据集总分看起来还行，但雷达图上某个维度已经明显偏低。

#### 公式的通俗解释

上边两组成分，合起来回答一个问题：**什么叫「这数据质量差」？**

- $Q(r)$ 回答「错的比例有多大」。低比例线性扣、中等比例加速扣、高比例指数扣——**惩罚速度随缺陷率变快，让严重问题不再能被轻度问题平均掉**。
- $C(c, N)$ 回答「错的绝对数量有多大」。小样本里5个无效SMILES和大样本里500个同样扣分更重——**小数据集的硬伤不会被稀释**。
- 木桶惩罚 $\Pi$ 回答「多块板子同时短」。一块板子短板扣一次；**超过三成板子都短，再叠加一笔**。

三者合在一起，总分就不再是「各项的平均」，而是「被最弱的一环拖下去的分数」——这也是MolJam取名为Cannikin Law的原因。
...
实验信息质量把MoleculeNet和ChEMBL两个家族彻底分开，**有没有实验注释，直接决定这个维度能得多少分**：所有MoleculeNet数据集因为没有实验元数据列，这个维度全部得0分；ChEMBL衍生集则因为有时间标签、检测方法信息等而获得较高分数。不过simpd系列只保留了时间标签，注释支持质量和类型多样性仍为0；而chembl_sarscov2保留了更完整的元数据，在这两个指标上得分更高。
...
> 同一把尺子量下来，两类基准的“差”方向相反：一个缺注释、一个缺广度——**它们不是同一种短板，不能只比总分高低**。
...
为把质量分数与模型性能联系起来，研究者用**18种机器学习算法×38种分子表示（共504种组合）**分别在原始和精修后的数据集上建模，以RMSE为主要指标。每个组合都按 **80/20随机划分训练集和测试集、跑5个不同随机种子**，再做配对t检验，确认精修前后的差异在5次划分之间保持稳健。**结果却并不一致**：Lipophilicity的改善率只有21.4%（504种组合中仅108种在精修后RMSE更低），中位数RMSE从0.943微升到0.950；ESOL改善率35.7%，但中位数RMSE也从1.213升到1.229。配对t检验显示，大多数组合在精修前后没有显著差异。

精修在提高质量的同时，也砍掉了数据量：ESOL从1128条降到908条（减少19.5%），Lipophilicity从4200条降到3608条（减少14.1%）。**质量上去了，信息量下来了，两者相互抵消**。

> MolJam分数衡量的是数据集质量，**不是数据集对你的任务有多好用**——它诊断数据，不保证模型受益。
...
为了把“质量效应”和“数量效应”拆开，研究者从精修后的Lipophilicity出发，设计了三种扰动策略，**用控制变量把质量和数量各自的影响单独拆开**：strip（保留分子但抹去立体化学标记，质量下降、数量不变）、remove（直接删除这些分子，质量上升、数量下降）、random remove（随机删除等量分子，数量下降、质量大致不变）。扰动比例取0%、5%、10%、15%、20%五个梯度，每个梯度下三种策略都用3个不同随机种子挑选要扰动的分子，再按上述80/20随机划分协议评估。
...
> 随机删样本只掉约4%，专挑不完整分子删却掉9%——**留着信息不完整的分子，反而比删掉它们对模型更有利**。
