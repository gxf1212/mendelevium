...
本附录补充主文档中未展开的技术细节，包括Y-scrambling验证、bootstrap置信区间分析、scaffold划分的详细统计量、适用域敏感性分析，以及残差分析的完整结果。
...
**图4：四个靶点上RF与Top竞争算法的$R^2$差异的bootstrap 95%置信区间**。绿色柱表示RF优势具有统计显著性（CI不包含零），灰色柱表示差异不显著。RF在绝大多数靶点-算法对比中均展示了显著优势

**四个靶点上，RF对所有Top-4竞争算法的优势均在95% CI下统计显著**（CI不包含零），这一结论不受随机种子选择的影响。
...
| DRD2 | 7558 | 3549 | 2501（70.5%） | 69 | 6036 | 1522 | 20.1% | +0.008（p = 0.25） |
...
- **DRD2的pActivity偏移最小**（+0.008，KS p = 0.25），说明训练集和测试集在活性分布上高度一致，其较小的scaffold gap（0.084）不能归因于分布偏移
...
**图14**：EGFR上RF + ECFP4的残差分析。（a）残差 vs 预测值散点图：x轴为预测pIC$_{50}$（4–10），y轴为残差（观测 − 预测），红色虚线为零残差参考线。数据点（浅蓝色）围绕零线随机分布，无系统性偏差。（b）残差分布直方图（MAE = 0.520）：蓝色柱为频率分布，红色虚线为均值残差位置，分布近似对称

EGFR上RF + ECFP4（随机划分）的残差分析显示：

- **无系统性偏差**：残差在pActivity预测值4–10的范围内围绕零线随机分布，无明显的趋势或模式
- **MAE = 0.520** pIC$_{50}$单位，即平均预测误差约半个log单位
- **残差分布近似对称**：直方图呈近似正态分布，峰值在零附近
- **活性极端区域的方差略有膨胀**：在pActivity < 4.5和 > 10.5的区域，残差离散度增大，这与训练数据密度在这些区域的稀疏性一致

其他三个靶点（DRD2、BACE-1、hERG）的残差分析呈现类似模式，均无系统性偏差。DRD2的MAE = 0.440，BACE-1的MAE = 0.518，hERG的MAE = 0.434，均在可接受范围内。

## 活性悬崖的完整描述符对比

完整的活性悬崖分析结果（RF，随机划分，seed = 42），cliff对定义为ECFP4 Tanimoto ≥ 0.6且|ΔpActivity| ≥ 2.0：
...
**ECFP4在所有四个靶点的cliff化合物预测上均表现最佳**，$R^2_{cliff}$范围从0.476（DRD2）到0.632（EGFR）。MACCS的cliff预测$R^2$始终最低，尤其是对DRD2（$R^2_{cliff}$仅0.316），进一步印证了预定义结构键指纹在精细结构区分上的局限性。

hERG的cliff密度最低（10.8%），且cliff与非cliff化合物的$R^2$差距也最小（ECFP4：$\Delta R^2$ = 0.103），这与其较窄的pActivity分布（SD = 0.91）一致——活性范围有限意味着“cliff”（|ΔpActivity| ≥ 2.0）的绝对数量较少。
