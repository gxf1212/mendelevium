...
![fig1](qhts_qsar_herg_figs/fig1.png)
...
```mermaid
graph TB
    subgraph S2["Model 2：预先排除策略"]
        direction TB
        B1["Tox21 10K数据集"]
        B2["识别hERG抑制剂<br/>并从GPCR活性中排除"]
        B3["8个GPCR数据集<br/>已排除hERG抑制剂"]
        B4["训练8个GPCR模型<br/>只包含非hERG抑制剂的GPCR活性化合物"]
        B5["预测GPCR活性<br/>默认低hERG风险"]
        B1 --> B2
        B2 --> B3
        B3 --> B4
        B4 --> B5
    ...
    subgraph S1["Model 1：分别建模策略"]
        direction TB
        A1["Tox21 10K数据集"]
        A2["8个GPCR数据集<br/>ADRB2/CHRM1/DRD2/HTR2A<br/>激动剂+拮抗剂"]
        A3["hERG抑制剂数据集"]
        A4["独立训练9个模型<br/>8个GPCR模型 + 1个hERG模型"]
        A5["独立预测<br/>GPCR活性 + hERG抑制"]
        A1 --> A2
        A1 --> A3
        A2 --> A4
        A3 --> A4
...
![fig2](qhts_qsar_herg_figs/fig2.png)
...
![fig3](qhts_qsar_herg_figs/fig3.png)
