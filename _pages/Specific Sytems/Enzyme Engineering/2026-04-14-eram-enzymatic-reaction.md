...
**图2：嵌入空间可视化**。T-SNE二维可视化展示（A）我们模型的表示和（B）ESM-2的表示；每个点代表一个酶的嵌入表示，随机选择15个酶类别进行高亮显示。（C）ERAM学习的酶（灰色）和小分子（红色）表示的T-SNE二维可视化。（D）酶和（E）小分子表示的正态分布也一并给出。

图2提供了关键的直观证据：**ERAM学到的表示比ESM-2更加语义紧凑和一致**。如何看出来的？对比图2A和图2B，随机高亮的15个酶类别（用不同颜色标记）在ERAM表示空间中**形成清晰的聚类边界**——相同颜色的点紧密聚在一起；而在ESM-2表示空间中，这些相同颜色的点**混杂在一起、边界模糊**。这说明ESM-2虽然学到了序列信息，但**没有充分学到酶的功能语义**。ERAM通过三元组对比学习，强制模型让底物—酶—产物关系成立，从而把功能相近的酶真正聚在一起。

更重要的是，图2C展示了**酶和小分子被映射到两个不同的子空间**：酶形成紧密的灰色聚类，小分子则更加分散的红色点包围着酶聚类。

> 这证实了双粒度对比学习成功捕捉了酶和小分子之间的功能差异——单纯替换产物会让整个反应变得不可行（知识图谱三元组中产生巨大几何缺口），而替换酶只影响反应速率而不影响可行性（产生的几何缺口较小）。图2D和E的正态分布也定量证实了**小分子的模显著大于酶的模**，这与双粒度设计的预期完全一致。
