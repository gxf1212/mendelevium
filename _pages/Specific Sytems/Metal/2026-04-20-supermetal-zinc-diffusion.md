...
> 金属离子是大量蛋白质中不可或缺的辅助因子，对酶活性和蛋白质相互作用至关重要。鉴于其关键作用和催化效率，**准确、高效地识别金属结合位点**对阐明其生物功能至关重要，并对蛋白质工程和药物发现具有重要意义。为应对这一挑战，本文提出了SuperMetal，一种利用**基于得分的扩散模型与置信度模型**相结合的生成式AI框架，能够高精度、高效率地预测蛋白质中的金属结合位点。以锌离子为例，SuperMetal优于现有最先进模型，实现了**94%的精确率和90%的召回率**，锌离子定位在实验确定位置的 $0.52 \pm 0.55$ Å范围内。SuperMetal展示了快速预测能力（约2000个残基的蛋白质不到10秒），且**不受蛋白质规模增大的显著影响**。值得注意的是，SuperMetal不需要关于金属离子数量的先验知识（不同于AlphaFold 3），且框架在原理上可扩展至其他金属离子或用作探针框架来识别其他类型的结合位点，如蛋白质结合口袋（但**目前仅在锌离子上充分验证**）。
...
- **精度已接近极限**：MAD中位数0.23 Å（高置信度）已接近X射线晶体学分辨率极限（1.5-3.0 Å），这是合理的，因为训练数据来自PDB实验结构，预测精度**无法超越源头**
...
- Abstract称"可扩展至其他金属离子或用作探针框架识别其他类型结合位点"，但**实际情况**：仅在锌离子上充分验证；SI提到SuperWater（水分子），但其他金属数据完全缺失
- **适用范围未清楚限定**：
...
- **ZincBind非冗余是位点级别**：ZincBind按结合位点的结构相似性聚类去重，但同一蛋白的多个构象如果锌结合位点有差异，会各自保留。这意味着随机划分时 homologous protein 可能跨越训练/测试边界
- **测试集的"similar"约束范围有限**：原文说"none of these 350 test structures contain binding sites similar to those in the training sets"——只针对350个测试结构相对于训练集，而8900个训练结构和1000个验证结构之间没有说明是否有交叉。若验证集和训练集之间有结构相似的蛋白，训练过程中的超参数调优会间接过拟合到这些蛋白，影响最终评估的公正性
