...
**图3：四种代表性原子对势函数**。图 3 把前面的统计规律落实到了具体势函数上：
...
- (d) **Ca−O.co2 更偏好单齿或对称双齿配位**，在约 `2.2–2.4 Å` 处有主极小值，并在约 `4.5 Å` 附近出现次级势阱，说明 Ca 更偏好单齿或对称双齿羧酸配位，而不是 Zn 那种更连续的羧酸配位范围

![fig4](metalkb_figs/fig4.png)

**图4：不同能量阈值下的 precision–recall 变化**

- 这里扫描的不是团大小阈值或配位距离阈值，而是**不同能量阈值**对预测表现的影响
...
图5b 还展示了**空间定位精度**。MetalKB(1.7) 的平均坐标误差是 `1.117 ± 1.567 Å`，表面上高于 Metal3D 在 `p = 0.75` 时的 `0.710 ± 0.631 Å`。但 MetalKB 的**中位误差只有 `0.224 Å`**，反而优于 Metal3D 的 `0.508 Å`。这与多核锌位点有关：因为两个真实锌离子本来就可能相距很近，误差统计容易被这些特殊案例拉高。
