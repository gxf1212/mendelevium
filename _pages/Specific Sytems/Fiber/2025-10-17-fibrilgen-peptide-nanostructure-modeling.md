...
cross-β结构的基本特征已为人所知：肽组装成单向的cross-β梯状结构，根据肽的相对取向可以是平行或反平行排列，片内主链肽间距为4.8-4.9 Å，并通过分子间氢键稳定。尽管在肽分子水平上组装相对简单，这些自组装肽可以形成具有**多种形态的扩展超分子组装体**，从细纤维到粗纤维、管状、带状和片状。尽管使用冷冻电镜、X射线衍射和固态核磁共振等最先进技术在阐明这些结构的形态方面做了大量工作，但**肽序列与最终超分子结构形态之间的关联仍然知之甚少**。不仅最终自组装结构取决于肽序列本身，介质pH值和离子强度、溶剂极性和温度等环境因素也在决定最终超分子组装体的形态中起着关键作用。
...
- **参数化建模方法**：通过7个几何参数（N, K, M, θs, θz, ry, θy）系统性地控制纤维形态，涵盖杆状、带状、管状等多种结构
...
FibrilGen的核心概念是**2×2肽单元**，即4条肽链组成的基本构建块，包含两个β折叠形成双层结构。pep2unit脚本提供三个关键控制选项：
...
| θz | 倾斜角，使单元偏离纤维轴（度） | ✓ | ✓ |
| ry | 孔径半径，单元距纤维轴的位移（Å） | ✓ | ✓ |
| θy | 扭曲角，沿纤维轴旋转连续单元（度）<br/>符号：+1为左手性，-1为右手性 | ✓ | ✓ |

**螺旋扭曲的数学关系**：

为了保持相邻肽间的氢键距离，扭曲角θy、倾斜角θz和半径ry必须满足几何约束：
...
2. **无空间冲突**：$0 < \theta_y < \theta_{y,\max}$，其中θy,max通过逐步增加扭曲角直到出现原子间距小于阈值来确定
3. **适当的片间距离**：相邻β折叠间的最近原子距离在2-5 Å之间

迭代过程：
- 对于**杆状结构**：从用户输入的θz开始，若不满足条件1和2，则以0.02 rad的步长逐步减小θz
- 对于**带状结构**：同时调整θz（步长0.02 rad）和ry（添加≤1 Å的随机噪声），直到满足所有三个条件
...
**图1：HP8管状结构的冷冻电镜密度与FibrilGen模型对比**

HP8肽（Ac-FKFEFKFE-NH2）自组装成10层β折叠的管状纳米结构，分辨率为3.2 Å。该管由内壁的平行β折叠和外壁的反平行β折叠组成。
...
**图2：AL1杆状结构的冷冻电镜密度与FibrilGen模型对比**

AL1肽（IGSNVVTWYQQL）形成12层平行β折叠的杆状结构，冷冻电镜分辨率8.3 Å，固态NMR确认平行排列。
...
**图3：Aβ42杆状结构的冷冻电镜密度与FibrilGen模型对比**

Aβ42肽是阿尔茨海默病的标志性淀粉样蛋白，组装成双折叠杆状结构，冷冻电镜分辨率4.0 Å。
...
**三个体系的共同特点**：FibrilGen模型在整体形态（管状、杆状）、螺旋参数（θz、θy）和骨架堆积方式上均与实验高度一致，证明了该方法的普适性。
...
除了上述三个验证案例，FibrilGen还展示了构建多种形态的能力（支持信息图S10-S11）：

**图7：FibrilGen支持的多样化形态**

1. **扁平片状结构**：θz和θy接近0
2. **细杆状纤维**：小的K矩阵（如2×2）+ 中等θy
3. **粗杆状纤维**：大的K矩阵（如3×4）+ 小θy
4. **紧密管状结构**：小M值（如M=4）+ 大θs（如90°）
5. **宽松管状结构**：大M值（如M=10）+ 小θs（如36°）
6. **左手/右手螺旋**：通过θy的符号控制（+1左手，-1右手）
...
- **A2**: 这反映了不同体系的**能量景观特征**：
  - **AL1杆**：12层平行β折叠的杆状结构具有**较宽的能量阱**，多种(θz, θy)组合在FibrilGen的空间冲突筛选中都可行。例如(θz=7°, θy=1.0°)和(θz=11°, θy=1.5°)都不产生冲突。
  - **HP8管**：10层混合平行/反平行β折叠的管状结构具有**更窄的能量阱**，内壁和外壁的不同排列方式对几何参数更敏感，只有θz≈25-30°和θy≈3-4°能同时满足内外壁的氢键和侧链堆积要求。
...
- **机器学习辅助设计**：训练神经网络从序列直接预测最优几何参数(N, K, M, θs, θz, ry, θy)，减少人工试错
- **自由能景观绘制**：对关键体系（如HP8）系统性扫描θz-θy空间，计算每个点的溶剂化自由能，绘制完整的形态相图
