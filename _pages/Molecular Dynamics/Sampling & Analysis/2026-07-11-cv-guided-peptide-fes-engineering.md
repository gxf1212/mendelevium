...
**图3**：残基重要性预测与热力学稳定性的强相关。（a）散点图显示WT HLDA残基权重（横轴）与每个位点突变引起的平均熔点变化$\Delta T_m$（纵轴）呈强负相关（Pearson $r = -0.98, p = 8.09 \times 10^{-5}$，$n = 7$）。每个数据点代表一个残基位点，颜色和符号区分不同氨基酸类型。（b）按残基显示的WT权重条形图，经过归一化处理（除以最大权重），显示不同位点的贡献差异；P3和Y9的权重较高，D2和E4较低。

### 特征值变化预测稳定效应

突变引起的HLDA最大特征值变化$\Delta\lambda$与$\Delta T_m$呈正相关（$r = 0.66, p = 1.1 \times 10^{-5}$）。这意味着**增加折叠-展开可分性的突变倾向于稳定该小肽**。
...
**图4**：特征值变化$\Delta\lambda$预测突变效应。（a）36个突变体的HLDA特征值变化$\Delta\lambda$（横轴）与熔点变化$\Delta T_m$（纵轴）呈正相关（Pearson $r = 0.66, p = 1.1 \times 10^{-5}$，Spearman $\rho = 0.61, p = 7.6 \times 10^{-5}$）。去除两个离群点D2R和T7D后，SI Fig.S5给出Pearson $r = 0.77$、Spearman $\rho = 0.74$。（b）子采样分析随机移除40%的突变体、重复100次；相关系数分布中心仍为$r = 0.66$，表明这种关系在该数据扰动下保持稳定。
...
CV-FEST相比传统REMD方法的显著优势：**表2 CV-FEST与传统REMD方法对比**

### 计算成本低

- **短无偏MD替代长REMD**：CV-FEST每个体系只需两条100 ns无偏MD（340 K下折叠态与展开态各一条）训练HLDA，无需对每个突变都做多副本高温REMD，单个突变的计算量显著降低
- **突变效应通过$\Delta\lambda$作廉价代理**：每个突变体的$\Delta\lambda$来自各自短无偏MD训练的HLDA主特征值与WT的差值，无需对每个突变都做完整REMD采样
- **总体成本节省**：36个突变体若逐一采用REMD，需要36次独立多副本模拟；CV-FEST则为各突变体开展短无偏MD与HLDA分析。**两者都随突变数线性增长**，CV-FEST的优势在于单个突变的计算成本远低于完整REMD，使大规模突变筛选变得可行。
...
1. **敏感位点预测**：通过WT的HLDA权重，可以初步识别哪些残基位点对突变可能更敏感，**帮助研究者优先关注这些位点进行实验验证**
2. **突变效应筛选**：通过$\Delta\lambda$可以初步预测哪些突变可能稳定或去稳小肽，**这种计算预筛选有助于缩小需要实验验证的突变范围**，但位点级$r=-0.98$仅基于7个位点平均值，预测可靠性需谨慎评估
3. **实验设计指导**：帮助实验学家优化突变库设计，**特别是在进行饱和突变扫描时，CV-FEST可以作为初步筛选工具**，但仍需实验验证
...
- **实用价值**：为蛋白质工程实验设计提供了计算筛选工具，**有助于减少需要实验验证的突变数量**，但$r=-0.98$仅基于7个位点的平均值，仍需在更多体系上验证
...
CV-FEST框架展示了如何用调和线性判别分析（HLDA）提取的集体变量来关联小肽突变效应。本文的核心贡献是：

- **方法论创新**：将HLDA用于自由能面工程，从每个体系短的无偏模拟中学习判别性CV。与需要大量数据的传统机器学习方法不同，CV-FEST可以在数据稀缺的情况下工作，这使其特别适用于实验数据有限的体系。
- **物理洞察**：发现HLDA权重与突变效应的强相关性（$r=-0.98$，$n=7$位点平均），提示折叠-转变贡献大的残基在WT折叠态和展开态之间变化显著。这种相关性依赖于特定的训练数据阈值选择，需在更多体系上验证。
- **实用价值**：提供了一个计算成本较低的筛选工具，可辅助小肽工程实验设计。CV-FEST的可解释性强，HLDA权重和特征值都有明确的物理意义，但$r=-0.98$基于仅7个位点的平均值，稳健性仍需进一步检验。

**特征值变化$\Delta\lambda$是一个与$\Delta T_m$相关的廉价代理指标**，可用于初步筛选对突变敏感的位点，但每个体系仍需短无偏MD训练HLDA。这种方法在CLN025小肽上展示了潜力，也为更大系统的蛋白质工程提供了有希望的思路。
