...
> 适用范围说明：本文主要讨论常规MD在机制解释中的用法。对于资源受限或项目早期阶段，这是最常见的起点；若需要更严格的定量结论，仍应结合增强采样或自由能计算。
...
关键点是**轨迹必须相互独立**：不同的初始速度、从不同构象出发，这样才能估计概率分布。如果只跑一条轨迹然后说“αC从out变成了in”，无法判断这是系统性的偏移还是随机涨落。更稳妥的做法是**从多个可信的实验构象作为补充起点**，检查结论是否依赖初始结构。PAK1那篇用了3个独立400 ns轨迹，是很好的同行评审可接受先例。
...
αC-in构象的一个关键稳定因素是**β3 链上的Lys残基和αC-helix上的Glu残基**之间形成盐桥。用这两个残基的**侧链带电原子间最小距离**（Lys Nζ–Glu Oε1/Oε2）或侧链重原子最小距离来量化。
...
这个指标衡量的是**apo体系是否已经采样到接近功能态/结合态的口袋构象**。如果apo构象集合中就有相当比例处于bound-like几何，说明口袋本身具有一定预组织程度，结合过程不需要付出大的构象熵代价。

具体的量化方式：定义ATP口袋的若干关键残基（如P-loop上的残基、αC-Glu、hinge区域残基），计算它们到配体上对应接触点的距离，然后和参考的晶体结构比较RMSD，**在apo体系中统计该距离落在结合构象范围内的采样概率**。holo体系则用于判断配体结合是否进一步稳定这种预组织构象。
...
> 对普通MD，可将概率分布转换为$-k_BT\ln P$用于直观展示采样到的构象landscape，但除非采样充分并验证收敛（如PT-metaD），不应把它解释为已经收敛的平衡自由能面。
...
**3条独立400 ns MD**（OpenMM，Amber ff14SB + GAFF）追踪三个量，三类量独立一致地指向同一个结论：**PAK1-A1维持PAK1的DFG-in/active-like kinase-core构象，同时更关键地重排DEK位点附近相互作用并促进kinase inhibitory segment释放**；而NVS-PAK1-1则明显稳定DFG-out非活性构象。

- **DFG motif扭转角分布**：PAK1-A1保持**DFG-in**（apo本身主要是DFG-in，结合后DFG几何只有轻微变化并很快回到baseline），NVS-PAK1-1（抑制剂对照）明显锁定**DFG-out**；
...
方法：将配体的关键接触残基突变（比如把形成氢键的Asp突变成Ala），重新跑holo模拟。如果**突变后构象分布回到apo的水平**，说明原来的偏移确实是由这些相互作用驱动的。但突变MD本身仍然是计算证据，最理想的验证是同时有实验活性数据，PAK1那篇正是这样做的。这个策略和文献中的药物抗性突变分析是一致的：如果某个突变导致抑制剂失效，模拟应该显示突变后的构象分布回到apo状态。
...
- 分布偏移的方向和活性之间的定量关系不是通用的。**不同激酶可能有更复杂的构象-活性映射**，不能简单套用
...
- 判断分布偏移是否显著，应检查**replica间是否一致**、**state population difference的置信区间**、**block/bootstrap误差**，以及必要时**Wasserstein/JS distance等分布差异指标**
