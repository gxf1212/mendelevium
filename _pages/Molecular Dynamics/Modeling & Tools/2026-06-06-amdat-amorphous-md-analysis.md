...
## 摘要
> AMDAT（Amorphous Molecular Dynamics Analysis Toolkit）是一个**开源C++工具包**，用于对分子动力学（MD）轨迹进行后处理，重点支持**非晶态、玻璃态与聚合物材料以及复杂流体（包括过冷液体）的高性能静态与动态分析**。本文介绍AMDAT的两个核心设计思路——**内存中的轨迹处理**与**指数时间采样**——如何实现高效的长时标分析，并以**径向分布函数**（**RDF**）、**结构因子**、**中间散射函数**（**ISFS**）及**邻居相关函数**为例展示其典型工作流。
...
- **可观测物理量齐全**：RDF、$S(q)$、ISFS、自Van Hove函数、邻居去相关函数、非高斯参数等一应俱全，**Simmons组内部用了十几年，支撑过好几篇聚合物和过冷液体的论文**
- **格式与脚本友好**：原生支持LAMMPS dump/xyz，对GROMACS xtc支持有限；输入脚本支持循环、条件、变量赋值，方便批处理和复用

## 背景

过去30年分子动力学模拟方法学已相当成熟，**GROMACS、LAMMPS、NAMD、AMBER、HOOMD-blue、OpenMM**等主流引擎在速度、可扩展性、力场支持上持续完善。但分析端是另一回事。通用工具（如MDAnalysis、OVITO）覆盖面广却偏通用，专门为非晶态、玻璃态、复杂流体设计的分析包仍然稀缺。这类体系的弛豫时间极长，线性采样的轨迹在长延迟处几乎无帧可用，短延迟处则严重过采样；RDF/$S(q)$等结构量看似成熟，但**邻居判定标准、Voronoi vs 距离截断的差异、长时自相关函数的统计**这些细节往往需要研究者自己写脚本。
...
> 说白了，AMDAT干的就是**MD引擎跑完轨迹之后的分析活**。LAMMPS/GROMACS输出的轨迹扔给它，出图出数。过冷液体、玻璃化转变、聚合物慢弛豫这些领域，时间尺度动辄跨十几个数量级，**一个能”指数地看时间”的分析工具基本是刚需**。
...
- **`trajectory list`**：一组粒子随时间的轨迹，可静态（固定粒子集）或动态（成员随时间变化的邻居集），是AMDAT的**核心数据对象**
...
> 用起来就像搭乐高：**分析程序就是把这几种对象堆起来**。比如"按物种1的邻居去相关率"：先从trajectory list选species 1 → 计算邻居形成neighbor list → 沿时间窗算邻居数变化 → 输出value list。**每一步的中间结果都可以接着用、接着叠加新的分析**。
...
**图1：三个体系的静态结构表征**。上行为径向分布函数$RDF(r)$，下行为静态结构因子$$S(q)$$。binLJ（左）和PS-30mer（右）的RDF按"全粒子/物种1/物种2/物种1-2对"分开绘制，**颜色：蓝=all、橙=1、绿=2、红=12**。KG（中）只显示全粒子RDF，因为它是单组分系统。$S(q)$三体系均按全粒子计算，**展示RDF与$S(q)$在不同波数/距离尺度上的互补信息**。
...
**图2：四体系（binLJ、binLJ2D、KG、PS-30mer）从上到下的四种动力学可观测**。

- **MSD**（均方位移）刻画扩散和亚扩散行为，binLJ2D的MSD在所有时间尺度上保持低斜率，**呈现典型的二维玻璃动力学**；PS-30mer在长延迟处出现Rouse-like或 reptation斜率。
- **ISFS**（self中间散射函数，$F_s(q, \tau)$）在对应近邻距离的波数$q^*$处计算，binLJ和PS-30mer能清晰看到$\alpha$-弛豫平台，KG在长延迟处尚未完全弛豫。
- **NGP**（非高斯参数$\alpha_2(\tau)$）的峰值位置标识**动态不均匀性最强的时间尺度**，binLJ2D的峰值最高，说明二维体系动态异质性最显著。
- **NDF**（邻居去相关函数）追踪局部环境在时间上的"换边"速度，**短时平台反映笼蔽效应**，长时衰减反映$\alpha$-弛豫。**颜色：蓝=all、橙=1、绿=2**，按物种切片。
...
> **Van Hove函数背景**：$G_s(r, \tau)$是单粒子时空相关函数，定义为一个粒子在时间$\tau$后位移为$r$的概率。它与**中间散射函数**$F_s(q, \tau)$互为傅里叶变换对：前者是实空间位移分布，后者是倒空间密度涨落衰减。Van Hove函数对非高斯扩散（如跳跃运动、协同重排）比MSD更敏感，因为MSD只看二阶矩（均方），会丢失分布形状信息。
...
- **AMDAT论文**：[arXiv:2602.05865](https://doi.org/10.48550/arXiv.2602.05865)
- **代码仓库**：https://github.com/dssimmons-codes/AMDAT
- **开发者文档**：https://dssimmons-codes.github.io/AMDAT/
- **Kremer–Grest原始论文**：Kremer, K., & Grest, G. S. (1990). Dynamics of entangled linear polymer melts: A molecular-dynamics simulation. *Journal of Chemical Physics*, 92(8), 5057-5086. https://doi.org/10.1063/1.458541
- **Van Hove函数教材**：[Van Hove Function for Diffusion in Zeolites](https://www.physik.uni-leipzig.de/~rhaberlandt/PDF/GaFrHa99.pdf) (莱比锡大学教程)
- **Van Hove函数应用**：[Local self-motion of water through the Van Hove function](https://link.aps.org/doi/10.1103/PhysRevE.102.032604) (Phys. Rev. E 2020)
