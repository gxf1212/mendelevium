...
> 前置文章：**[AMDAT：面向过冷液体与玻璃态体系的长时标MD分析工具](https://mendelevium.github.io/Molecular%20Dynamics/Modeling%20&%20Tools/2026-06-06-amdat-amorphous-md-analysis.html)** 介绍了AMDAT的基础功能（RDF、MSD、ISFS、Van Hove、per-particle分析）。本文是其**进阶篇**，重点演示AMDAT对**空间分辨分析**的能力——这在多组分、非均匀体系（如PNC、表面、界面）的研究中至关重要。
...
> 核心思想：**这五种分区都是"动态定义"的**——它们基于实时几何关系计算，而非预设固定距离阈值。例如"interfiller"区域的存在与否取决于两个填充分散物团簇的相对位置，AMDAT在每一帧都能自动重新计算。
...
> 关键观察：**interphase shells的ISFS明显滞后于bulk polymer**——这正是”界面层动力学慢于体相“的实验观察的MD模拟证据。**filler和centers几乎不动**，说明它们是刚性粒子。**contacting filler的ISFS比filler本体还慢**，说明”紧束缚“层的聚合物反而”锁住“了部分填充分散物的运动。

### 三、AMDAT脚本的五个模块

实现图10的分析需要五个AMDAT算法模块（Algorithms 7-11），展示了AMDAT的**模块化工作流**：
...
> 可视化的价值：**图12b清楚显示，填充分散物表面的红色"接触层"分布不均匀**——某些团簇表面有大量接触，某些几乎没有。这说明**"填充分散物-聚合物相互作用"在空间上高度不均匀**，这对理解PNC的机械强度至关重要。
...
> AMDAT的模块化设计让这些方法可以"组合"使用。例如，你可以先用`create bin list with options distance`定义界面层，再用`find_between`识别interfiller区域，然后分别计算ISFS，最后输出xyz文件在OVITO中可视化。**整个流程都在脚本中自动化**，结果完全可复现。

## 关键结论

- **空间分辨是AMDAT的杀手级功能**：`trajectory bin list`和`find_between`等命令让研究者能**基于几何关系和动力学特征自动定义空间区域**，无需手动指定阈值
- **动态定义 vs 静态阈值**：手动定义"距表面5 Å以内的原子"既不准确也不可扩展；AMDAT的动态定义基于**实时计算的距离关系和几何拓扑**，能自适应填充分散物团簇的非球形和空间分布变化
- **ISFS分区域计算的洞察**：图10清晰显示**界面层的ISFS明显滞后于体相聚合物**，填充分散物和中心的ISFS几乎不弛豫，这为"PNC界面层动力学慢于体相"的实验观察提供了**MD模拟层面的定量证据**
- **可视化与分析的无缝衔接**：AMDAT计算的区域标签、ISFS值、per-atom位移等都能导出为xyz/pdb文件的某一列，**直接导入OVITO/VMD按该列着色**，实现"分析"到"可视化"的一键式流程
- **可复现的分析管线**：整个分区→分析→可视化的流程都在脚本中完成，**不同研究者、不同轨迹都能用同一脚本得到完全一致的结果**，这对PNC这类复杂体系的研究尤其重要

> 实用提示：如果你研究的体系有**多个组分的非均匀分布**（如填充分散物、表面涂层、电解质界面），AMDAT的空间分辨分析能力几乎是**不可替代的**。手动定义区域既不准确也不可复现，而AMDAT的脚本化分析管线能让你**批量处理数百帧轨迹、自动输出每个区域的统计数据和可视化文件**。
...
- **代码仓库**：https://github.com/dssimmons-codes/AMDAT
- **开发者文档**：https://dssimmons-codes.github.io/AMDAT/
- **PNC相关工作**（AMDAT论文参考文献21、22、23）：Simmons组用这套方法研究填充弹性体的机械变形，证实了界面层的应力分布与填充分散物-聚合物相互作用的关系

---

**小编锐评**：AMDAT最打动我的是`find_between`这个命令——它用一个简单几何条件（”到A的距离 < 到B的距离“）就能识别”夹在两个团簇之间“的聚合物。这种**基于几何关系的动态定义**比手动框选不知道高明多少倍。对任何研究**非均匀体系**（PNC、表面、生物膜、嵌段共聚物）的人来说，这套工具值得认真学。
