...
模拟退火过程通过旋转单体内部的可旋转键来探索构象空间。PSP提供三种策略：
- **SA1策略：标准模拟退火**，不加拉伸，适用于大多数柔性聚合物
- **SA2策略：50 kJ/mol拉伸阈值**，加入拉伸阈值防止链塌缩，适用于中等刚性骨架
- **SA3策略：150 kJ/mol拉伸阈值**，加入更高的拉伸阈值，适用于骨架刚性较强的聚合物

如果单体的模拟退火反复失败，PSP会自动尝试用**二聚体替代单体**作为构建单元，增加骨架柔性，显著提高成功率。

**关键限制**：对于骨架中有双键的聚合物（如聚乙炔），ChainBuilder无法生成寡聚物（因为氢原子不能饱和连接位点的价态）。用户需要改写SMILES，让连接位点用单键连接。

#### CrystalBuilder：晶体结构预测

CrystalBuilder是PSP独有的能力（RadonPy和PEMD都不做晶体预测），采用**刚体采样**方式生成候选晶体结构。具体流程：

1. **链拷贝生成**：从ChainBuilder输出的无限链中提取一个晶胞拷贝
2. **刚体操作**：将链拷贝视为刚体，沿z轴放置，然后通过随机旋转和平移生成多个候选构型
3. **距离约束**：确保所有原子之间的间距不低于2Å，避免不合理的原子重叠
4. **能量评估**：用UFF力场快速评估每个候选结构的能量，筛选出能量较低的构型
5. **输出格式**：生成VASP POSCAR格式文件，可直接用于DFT计算
