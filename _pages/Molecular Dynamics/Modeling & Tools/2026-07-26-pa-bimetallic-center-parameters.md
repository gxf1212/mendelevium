...
要回答的问题很直接：在 apo 态和 BXA 结合态这两种不同需求下，哪种参数化策略更合适？已有研究通常只验证单一状态，跨态比较和对耐药突变的应用验证还不完整。
...
- **双金属中心的参数化对比**：在 apo 态和 BXA 结合态两个场景下，对非键、键合、杂化四类策略做了统一比较
- **BXA结合态的杂化处理**：在杂化方案里，BXA 和配位水保留 RESP 电荷，金属-配位水约束被移除，便于观察几何和交换行为的变化
- **五种突变的动力学分析**：基于 500 ns × 3 副本模拟，作者分别分析了 I38T/F/M、A36V 和 E23K 对口袋构象和配体接触的影响
- **数据与脚本公开**：参数、输入结构和模拟脚本都放在 GitHub 上，方便复现
...
三种突变都削弱了Ile38-BXA锚网络的“持久锁定”特征，将其转化为更动态、可交换的接触模式，导致BXA疏水wing在口袋内采样多个局部微构象，整体RMSF升高。SI中的apo态对比显示，I38T/F/M与WT的骨架RMSD和RMSF基本一致（Figure S4）。这些突变的效应出现在药物结合态，而不是一般性的结构去稳定。
...
PAN 的耐药机制不是单一突变破坏单一相互作用，而是**通过不同路径降低 BXA 的稳定性**：I38 突变削弱局部疏水锚网络，A36V 改变 apo 态口袋开放程度，E23K 通过静电重布线削弱 specificity wing。**不同突变对应的路径不一样，后续抑制剂设计也需要分别考虑**——针对单一机制的下一代 ceni 设计可能不足以覆盖所有耐药突变，需要针对不同物理路径设计差异化干预策略。
