...
- **中和**：添加$\mathrm{Na^+}$，再加入0.15 M NaCl模拟生理离子强度
...
PAN 的耐药机制不是单一突变破坏单一相互作用，而是**通过不同路径降低 BXA 的稳定性**：I38 突变削弱局部疏水锚网络，A36V 改变 apo 态口袋开放程度，E23K 通过静电重布线削弱 specificity wing。**不同突变对应的路径不一样，后续抑制剂设计也需要分别考虑**——下一代 ceni 需要从多个物理路径入手，单一机制设计难以覆盖所有耐药突变。
