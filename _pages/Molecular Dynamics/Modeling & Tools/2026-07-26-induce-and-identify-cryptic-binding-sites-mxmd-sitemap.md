...
> 在基于结构的药物发现中，识别蛋白上的隐藏结合位点仍然是一个挑战，因为这些位点在apo结构中通常看不出来。本研究开发并验证了一套将混合溶剂分子动力学（MxMD）模拟与SiteMap相结合的**诱导＋识别工作流**。该方法利用MxMD采样蛋白构象以暴露隐藏口袋，再由SiteMap有效地识别和排序这些口袋。在一个包含**65个隐藏结合位点**的具有挑战性的数据集上，所开发的工作流在**78.5%的案例中能在top-5预测中识别出隐藏结合位点**。这些结果表明，所提出的MxMD + SiteMap工作流为早期药物发现提供了一个稳健且有价值的工具，能够通过有效诱导和识别隐藏结合位点来探索更广泛的成药靶标。
...
蛋白的构象是动态的——很多配体结合位点只有在配体诱导下才会形成对应的构象，这些位点被称为**隐藏结合位点**（cryptic binding site）。它们对新药研发价值很高：既能解释已知结合剂的机制，也为虚拟筛选和别构调控打开了新的可能性。但传统上，隐藏位点主要靠**碎晶筛选**、**片段筛选**和**溶液NMR**这些实验手段偶然发现，而这些方法既贵又费时，通常还得先有已知结合物才能做。在没有已知结合物的情况下，用计算方法预先把隐藏位点挖出来，就成了结构药物发现早期的硬需求。
...
- **隐藏口袋在apo结构里根本不存在，怎么无中生有地把它找出来**：这是结构药物发现中长期悬而未决的硬问题。SiteMap、MxMD等单方法受限于采样或打分的局限，对需要大幅构象重排的隐藏口袋无能为力。
...
MxMD的核心思想是把**小分子探针**（默认乙腈、异丙醇、嘧啶）混入水溶剂中跑MD。探针分子会自发探索蛋白表面的各个角落，频繁结合到能量有利的位点——这些位点很可能就是潜在的结合口袋。Schrödinger官方协议包含两个溶剂化步骤和七个动力学步骤：
...
MxMD的探针占用率只是口袋开放过的信号，很多隐藏口袋可能根本没等到探针占领就关上了——这就是原MxMD工作流漏掉大量位点的原因。所以作者把MxMD的采样结果直接交给SiteMap——Schrödinger基于物理的打分工具，它能为蛋白表面每个候选位点计算一个**SiteScore**，综合考虑位点的体积、暴露度、疏水性、亲水性等物理特征。工作流的处理流程包括以下几个环节：

- **SiteMap两种模式选择**：在本文数据集上，作者先用**site-evaluation模式**（以holo配体为中心、6 Å半径的盒子）对apo和holo结构都跑了SiteMap，再用**site-detection模式**对apo结构做完整扫描——前者用来评估apo/holo的构象差异，后者用来发现新位点
- **跨帧位点提取**：对MxMD采样的所有约3000帧结构都用SiteMap在site-detection模式下跑一遍，得到约15,000个候选位点
- **去冗余合并**：用类似**Sphere Exclusion**的方法，按SiteScore倒序遍历，每选中一个位点就把与之高度重叠（残基Tanimoto相似度大于0.4）的所有位点都标记掉
- **DCC判据评估**：位点中心到任何配体原子的距离小于4 Å视为配体占据，4 Å阈值沿用自P2Rank文章
- **位点残基定义**：每个SiteMap位点对应的位点残基定义为距离位点中心3.5 Å以内的蛋白残基；肽和蛋白配体的位点-配体重叠只统计距受体3 Å以内的配体原子
...
- **横轴**：SiteScore区间（0-0.6, 0.6-0.8, 0.8-0.9, 0.9-1.0, 1.0-1.1, 1.1-1.2, 1.2+）
- **纵轴**：Ligand Affinity（pKi）
...
**图3：apo与holo结构构象对比（PDB: 3QXW VHH抗体）**

- 灰色cartoon为apo结构，蓝色ribbon为对齐后的holo结构，橙色sticks为holo配体——两个状态在配体结合区域有显著构象差异，配体口袋在apo状态基本关闭
- 这种apo/holo构象差异正是隐藏位点检测的根本挑战所在：传统基于单帧apo结构的位点评分方法天然看不到那个隐藏口袋
- 与原文Figure 5D一致；Figure 3原文对应的2OHG/3QXW SiteMap点位结果图未在此处提供
...
原文Figure 4用PDB 3UGK（酵母氨酸脱氢酶）展示了**MxMD的采样能力为什么重要**：3UGK的口袋在apo状态是关闭的，SiteMap的单帧打分看不到，但MxMD的探针在MD过程中撬开了口袋——在MxMD诱导出的瞬时构象中，SiteMap能识别出与holo配体重合的位点。这正是原文想强调的MxMD采样能力对识别隐藏口袋不可或缺的核心观点。

原文Figure 5用3QXW做了一组三联对比：SiteMap单独（图5A）、MxMD单独（图5B）、MxMD + SiteMap工作流（图5C），加上apo与holo的构象对比（图5D）——共同展示本文最重要的induce + identify思想：**SiteMap和MxMD单独都不行，但MxMD采样的某一帧构象开放之后，SiteMap就能识别出来**。本文的图3以3QXW的apo/holo构象叠加为代表，呈现了图5D的对比视角。
...
**Boltz-1**是个深度学习共折叠模型，从序列和候选配体出发预测蛋白-配体共折叠结构，**隐式把配体放到最可能的口袋里**。它在top-1命中率上达到**84.6**%（55个结构），平均配体-位点重叠0.88。但作者明确指出：**(1)** Boltz-1需要已知结合物作为输入才能用，但隐藏口袋场景下我们往往没有已知结合物；**(2)** Boltz-1训练截止日期是2021-09-30、分辨率上限9 Å，**本数据集65个系统里只有1MPU（holo: 8EA5）一个不在训练集里**，top-1性能可能被数据集重叠大幅夸大；**(3)** 共折叠模型偏向正构位点，对别构口袋不友好。所以Boltz-1的数字虽漂亮，但**前提条件差异太大**，和本文工作流不是直接对等的关系。

![fig6](jcim6c01288_figs/fig6.png)

**图4：KRAS蛋白apo结构上MxMD + SiteMap识别的位点（对应原文Figure 6）**

- **灰色cartoon**：KRAS apo蛋白的MxMD帧结构
- **橙色sticks**：对齐后的holo配体
- **绿色点**：MxMD + SiteMap工作流识别的位点
- **本图覆盖范围**：包括GTP/GDP位点、Switch II位点和Switch I/II位点（后两者是经典别构位点）
