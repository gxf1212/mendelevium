...
- **碎晶筛选**（HTS）：高通量筛选化合物库，靠结晶偶发发现
- **片段筛选**：用小分子片段探测蛋白表面潜在结合区域
- **溶液NMR**：用化学位移扰动捕捉配体与蛋白的弱相互作用

这些方法**既贵又费时，通常还得先有已知结合物才能做**。在没有已知结合物的情况下，用计算方法预先挖出隐藏位点就成了硬需求。计算方法主要分两条路：
...
探针分子会自发探索蛋白表面的各个角落，频繁结合到能量有利的位点——这些位点很可能就是潜在的结合口袋。Schrödinger官方协议包含两个溶剂化步骤和七个动力学步骤。前六步动力学是**逐步升温、去约束的平衡过程**，确保体系在开始生产模拟前温度、压力、密度都达到合理状态；最后两步才是真正的生产模拟：
...
MxMD内部对采样结果的处理——把探针的“访问频率”翻译成“口袋候选”：
...
工作流在top-5命中率上**比SiteMap单独使用提升了12.3个百分点**，top-10命中率上**提升了27.7个百分点**。剩下3个连top-10都没命中的案例（PDB: 3KFL, 3P53, 1FXX/3HL8）作者都做了调查：前两个的SiteScore低（0.9-1.0），但**top-ranking的工作流位点其实就在配体附近**，只是DCC判据（4 Å阈值）没判为“liganded”——属于“false negative”；第三个是**PPI位点**，默认SiteMap没针对PPI优化，换成PPI模式后排名第9。
...
**采样长度权衡**：把MxMD的采集段从默认的最后5 ns延长到最后**50 ns**（总生产段延长至65 ns，在22个位点子集上测试），采集帧数增加10倍。结果如下：

- **整体表现反而下降**：长轨迹产生更多高分位点（平均SiteScore从1.15升到1.21），false positive率同步上升
- **个别案例改善**：3KFL原本排名64，延长后升到12
- **默认协议更平衡**：15 ns生产 + 5 ns采集在采样效率和false positive之间已经是个相对好的平衡点，**采样成功和false positive之间需要平衡**

**Vanilla MD对比**：把探针去掉、只用水跑了vanilla MD，所有其他设置完全相同（10条重复、同样的模拟流程）。把65个位点按“是否被SiteMap单独从apo结构识别”分成44个“easy”位点和21个“hard”位点，细分结果如下：

- **整体表现与MxMD可比**
- **easy位点略好**：vanilla MD产生更多构象变化，已打开的位点更容易被识别
- **hard位点略差**：没有探针诱导，纯热运动难以撬开深埋的隐藏口袋

**MxMD的探针对真正难隐式位点的诱导是不可或缺的**。
