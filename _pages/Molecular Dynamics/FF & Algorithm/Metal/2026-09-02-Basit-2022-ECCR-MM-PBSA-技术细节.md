...
> Basit等（2022）的目标不是用严格的ABFE直接计算 $\ce{Ca^{2+}}$ 结合自由能，而是建立**低成本的CaM突变体亲和力预测模型**。
...
Asp/Glu的**两个羧酸O都修改**，**并不只修改晶体结构中直接朝向钙离子的那个O**。例如loop-3选了Asp93、Asp95（两个羧酸O均修改）、Asn97（侧链羰基O）、Tyr99（主链羰基O）、Glu104（两个羧酸O均修改），一共8个O。Loop-4选了Asp129、Asp131、Asp133（三个Asp共6个羧酸O）、Gln135（主链羰基O）、Glu140（两个羧酸O），一共9个O。

![图5 loop-3与loop-4结合位点的残基示意](./Basit-2022-ECCR-MM-PBSA_figs/fig5.png)

**图5：loop-3与loop-4结合位点残基排布与电荷补偿氧原子**。左图为loop-3，右图为loop-4，单字母缩写加残基编号标注各配位残基，W代表结合位点中的水分子。图中显示直接配位 $\ce{Ca^{2+}}$ 的氧原子包括Asp/Glu侧链羧酸氧、Asn97侧链羰基氧、Tyr99与Gln135主链羰基氧，以及配位水。ECCR电荷补偿即对这些预先指定的氧原子统一施加 +0.0556e到 +0.0625e的小增量，使蛋白总电荷保持整数。

迁移到其他EF-hand体系时，这一点尤其重要：

> ECCR的电荷补偿**不是「看到Ca附近几个O就自动重新拟合」的算法**，而是人为指定一组EF-hand邻近O再做总电荷补偿——迁移时补偿组要自己重新定义。
...
$$
\frac{\Delta G_\text{bind}}{2} = \dfrac{\Delta G_3 + \Delta G_{4,3} + \Delta G_4 + \Delta G_{3,4}}{4}
$$
...
![图2钙调蛋白C-lobe两个EF-hand的两步钙离子结合自由能定义](./Basit-2022-ECCR-MM-PBSA_figs/fig2.png)

**图2：钙调蛋白C-lobe两个EF-hand的两步钙离子结合自由能定义**。白色圆圈为空位点（左为loop-3），红色圆圈为已结合 $\ce{Ca^{2+}}$ 的位点；$\Delta G_{x,y}$ 表示当位点y已被占据时 $\ce{Ca^{2+}}$ 结合位点x的自由能变化。

![图S1钙调蛋白钙离子结合自由能计算流程图](./Basit-2022-ECCR-MM-PBSA_figs/si_figure_s1.jpeg)

**图S1**：从复合物轨迹提取受体与配体、四种site-specific过程到宏观结合自由能的完整流程。

---

## 结果到底有多好？

> 先立一个分界：**原始MM-PBSA结果**和**后面的回归拟合结果**是两回事，评价要分开，别被0.3 kcal/mol的表面精度带偏。
...
单位均为kcal/mol。对这几个体系，**标准 +2e钙离子明显过强结合**；简单的电荷缩放把结果大幅拉向实验值。
...
> 后面出现的RMSE ≈ 0.3 kcal/mol、$r \approx 0.8$，**对应的是回归校准后的结果，不是原始MM-PBSA**——两套数字不能混用。
...
它得到RMSE ≈ **0.34 kcal/mol**、Spearman $r \approx$ **0.80**、Pearson $r \approx$ **0.80**；多描述符模型的误差与它接近（主文称各模型RMSE与相关性都很接近），其 $R^2$ 为 0.698（SI Table S10）。
...
> **采样短不等于采样充分**：均值稳不稳和构象全不全没关系；迁移到其他EF-hand体系时，一旦涉及构象重排（磷酸化、变构），先怀疑轨迹长度。
...
迁移到其他EF-hand体系时，这篇真正能拿走的一点：如果pSer/pThr离 $\ce{Ca^{2+}}$ 第一配位层较远，通常不需要照这篇去改磷酸基；真正要决定的是，是否像他们一样在每个EF-hand第一配位环境附近的O上统一分摊 +0.5e。
