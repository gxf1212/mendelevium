...
图1：Duarte et al. 2014的CDA模型示意图。（A）虚拟原子模型：中心金属离子（蓝色）周围有6个虚拟原子（红色），虚拟原子与中心离子距离固定为0.9 Å，整体保持八面体几何。原文这里只明确写了恒定距离，没有单独给出角度或二面角参数。（B）人类乙二醛酶 I 活性位点结构，显示$\ce{Zn^{2+}}$与两个谷氨酸、两个组氨酸和水分子形成八面体配位。
...
- **先定骨架**：沿用已有的八面体 dummy 几何，原文只说做了小幅修改，没有给出具体数值，让配位框架先稳定下来。
...
**图5**：催化金属中心的配位球结构。（A）E. coli $\ce{Ni^{2+}}$-GlxI的活性位点，（B）人类$\ce{Zn^{2+}}$-GlxI的活性位点。CDA模型（蓝色球）成功替代真实金属离子，保持了八面体配位几何。
...
这个重新分配由**电负性均衡方法（EEM）**驱动，本质上是一个带约束的能量最小化问题。EEM能量函数为 $E_{\text{EEM}} = \chi^{\mathsf T}q + \frac12 q^{\mathsf T}Hq$：第一项 $\chi_i q_i$ 是驱动力，代表每个位点携带电荷时的电势能，电负性 $\chi_i$ 越低越倾向于失电子；第二项 $\frac12 q_i J_{ij} q_j$ 是代价，对角项 $J_{ii} = \eta_i$ 防止电荷无限制地堆到单个位点，非对角项 $J_{ij}$ 描述不同位点间的静电耦合，屏蔽参数 $\gamma_{ij}$ 防止短程库仑爆炸。在总电荷约束 $\sum q_i = Q_\text{total}$ 下，用拉格朗日乘子求解，最终等价于求解一个 $7 \times 7$ 的增广线性方程组，每步MD仅需一次线性代数计算。

之所以说它是**局部**动态极化，是因为只有核心7位点是动态电荷未知量——周围的水分子和配体提供瞬时外场，但不作为独立的动态电荷一起优化。若结合ReaxFF/AMBER论文里的mEEM写法来理解，这类外层固定电荷对核心区的作用可以写成类似 $H_{\text{core-trans}} q_{\text{trans}}$ 的驱动项；不过 CDApol 主文并没有把这一步单独展开成完整记号体系。

整个参数化流程分为两步：**第一步**训练EEM参数（电负性 $\chi_i$、硬度 $\eta_i$、屏蔽 $\gamma_{ij}$），以DFT计算的水合构象电荷分布为参考目标，让动态电荷分布落在合理范围；**第二步**扫描Lennard-Jones参数（$\varepsilon$ 和 $R_{\min}/2$），用热力学积分（TI）计算水合自由能（HFE），同时匹配实验HFE、离子-氧距离（IOD）和配位数（CN）三个目标。这两步串联进行：先定电荷分布，再调非键参数。

> 这套机制的技术细节（含完整公式推导、EEM物理图像、mEEM约束求解、双层筛选机制、两步参数化流程与TI实现）已整理为独立文章：[CDApol极化模型方法论详解](./2026-05-06-polarizable-CDA-methods)。
...
- 电荷重分布使虚拟原子一侧更能响应局部水合环境，从而有助于把IOD调回目标范围
...
**图4：扫描LJ参数得到的水合自由能结果**
- **上排**：$\ce{Zn^{2+}}$、$\ce{Al^{3+}}$和$\ce{Zr^{4+}}$的CDApol模型HFE扫描结果。
- **下排**：相同三种离子的固定电荷CDA模型HFE扫描结果。
- **坐标说明**：横轴是$R_{\min}/2$，纵轴是$\varepsilon$，每个点对应一组12-6 LJ参数。
- **颜色说明**：颜色表示该组LJ参数下计算得到的HFE绝对值，具体数值以每个子图右侧图例为准；颜色跨度越大，说明HFE对LJ参数越敏感。
...
CDApol的HFE误差均在**5%以内**，明显优于固定电荷CDA模型。
...
- **上排**：$\ce{Zn^{2+}}$、$\ce{Al^{3+}}$和$\ce{Zr^{4+}}$的CDApol模型IOD扫描结果。
- **下排**：相同三种离子的固定电荷CDA模型IOD扫描结果。
- **坐标说明**：横轴是$R_{\min}/2$，纵轴是$\varepsilon$，每个点对应一组12-6 LJ参数。
- **颜色说明**：颜色表示该组LJ参数下得到的IOD，具体Å数值以每个子图右侧图例为准；蓝色通常对应较短IOD，红橙色对应较长IOD。

IOD扫描展示了结构性质对LJ参数的响应。CDApol可以在合理参数区域同时接近目标M-O距离，而固定电荷CDA更容易出现距离偏短或偏长的问题。这也是本文强调HFE、IOD和CN需要一起验收的原因。

![fig6](2026-05-06-polarizable-cationic-dummy-model_figs/fig6.png)

**图6：经典AMBER、固定电荷CDA和CDApol的最终误差对比**
- **子图说明**：三个小图分别对应$\ce{Zn^{2+}}$、$\ce{Al^{3+}}$和$\ce{Zr^{4+}}$。
- **颜色说明**：蓝色柱表示HFE误差，橙色柱表示IOD误差，灰色柱表示CN误差。
- **横轴说明**：每个子图内比较经典AMBER、固定电荷CDA和CDApol三种模型。
- **纵轴说明**：百分比误差，相对于目标实验值计算。

图6把热力学和结构指标放在同一张图里比较。CDApol的关键优势不是只把某一个数值调好，而是在HFE、IOD和CN三个指标上同时降低误差；这正好对应高价金属离子固定电荷模型最难处理的地方。

> **CDApol的优势**：在本文测试的标准12-6 LJ模型框架下，同时把实验HFE、IOD和CN都拉回到较合理的范围，而固定电荷CDA模型在IOD和CN上偏离目标值较大
...
- **兼容性较好**：既能和TIP3P这类非极化水模型耦合，也能和OPC这类极化水模型一起用。
...
#### 为什么会出现$\ce{Al^{3+}}$的反直觉结果

$\ce{Al^{3+}}$的EEM优化里，中心离子会出现**负电荷**（-0.33），虚拟原子则更正（+0.55）。这是EEM按目标IOD重新分配电荷的结果。目标IOD越短，电荷分布就越倾向于把虚拟原子推到更靠近水分子氧原子的位置。
...
| CDApol | **优秀（接近实验）** | **优秀（接近实验）** | **优秀（接近实验）** | **中等（+35%）** | **待验证** |

CDApol在**准确性、效率和可迁移性**之间取得了平衡。
