---
title: "RNA二级结构与三维结构预测的相互作用：一项全面研究"
date: "2026-04-09"
...
- **发表时间**：2026年4月1日
- **单位**：浙江大学定量生物中心、温州大学物理系、国家生物药技术创新中心（苏州）
...
- **源代码**：预测结果可在 https://github.com/DongZhangRNA/2D-and-3D-benchmark 获取
...
对于所有六个三维方法，当F1分数从0.2增加到1.0时，RNA三维结构预测准确性的总体趋势呈上升态势（RMSD下降，INF_ALL上升）。
...
**RNAComposer**的预测有最多的情况（比例 = 0.54），其中F1分数没有变化，而**trRosettaRNA和NuFold**的预测有最少的情况（比例 = 0.07和0.09）。
...
这项研究最大的价值在于**系统性地揭示了一个被忽视的问题**：RNA三维结构预测过程中，输入的二级结构不是简单地被"遵守"或"违反"，而是会被模型主动地"修改"。而且这种修改能力在不同模型间差异巨大，直接影响了最终预测质量。

更妙的是，研究发现**假阳性碱基对比真阳性碱基对更"致命"**——这就像做选择题时，"不确定的空着"比"猜错了"危害更小。这个洞察非常实用，为未来的工具改进指明了明确方向：与其追求更多真阳性，不如先学会减少假阳性。

对普通用户而言，最实用的启示可能是：**如果你的二级结构预测质量一般（F1在0.55-0.65之间），trRosettaRNA可能比其他工具更合适**；但如果你有高质量的二级结构（比如从AlphaFold3提取的），那么传统方法如RNAComposer可能更可靠。

当然，研究也坦承了局限性：RNA结构预测工具迭代太快，今天的结论明天可能就过时了。但无论如何，这种系统性的基准测试和深入的机制分析，为整个领域提供了宝贵的参考框架。
