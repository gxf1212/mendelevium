...
- **功能多样性**：包括核酶、核糖开关、tRNA、rRNA等多种功能RNA
...
CASP RNA数据集涵盖14个在CASP15和CASP16竞赛中使用的RNA靶标。CASP（Critical Assessment of protein Structure Prediction）是蛋白质结构预测领域的最重要的盲测竞赛，从CASP15开始首次引入RNA靶标。

这些RNA靶标的特点包括：
- **合成RNA**：包含缺乏天然同源物的合成RNA序列
- **多样化结构**：涵盖不同的结构类型和长度
...
| 模型 | 中位数RMSD (Å) | 中位数INF_ALL | 中位数TM-score | 中位数lDDT |
| --- | --- | --- | --- | --- |
| **trRosettaRNA** | 15.8 | 0.72 | **0.27** | 0.55 |
| **NuFold** | 18.2 | 0.68 | 0.21 | 0.48 |
| **RNAComposer** | **14.3** | **0.77** | 0.25 | 0.52 |
| **FARFAR2** | 16.5 | 0.70 | 0.23 | **0.59** |
| **IsRNA2** | 17.1 | 0.69 | 0.22 | 0.51 |
| **SimRNA** | 17.8 | 0.68 | 0.20 | 0.49 |
...
| 模型 | 中位数RMSD (Å) | 中位数INF_ALL | 中位数TM-score | 中位数lDDT |
| --- | --- | --- | --- | --- |
| **trRosettaRNA** | **2.7** | **0.77** | **0.57** | **0.66** |
| **NuFold** | 4.2 | 0.68 | 0.42 | 0.55 |
| **RNAComposer** | 3.5 | 0.72 | 0.48 | 0.58 |
| **FARFAR2** | 3.8 | 0.70 | 0.45 | 0.56 |
| **IsRNA2** | 4.0 | 0.69 | 0.44 | 0.54 |
| **SimRNA** | 4.5 | 0.65 | 0.40 | 0.52 |

### 在CASP RNA数据集上的详细表现

对于CASP RNA数据集，**trRosettaRNA**同样在六个所选模型中排名第一（中位数RMSD = 9.9 Å，中位数INF_ALL = 0.82，中位数TM-score = 0.49，中位数lDDT = 0.66）。CASP RNA数据集包含合成RNA，这些RNA缺乏天然同源物，对三维结构预测方法提出了严峻挑战，trRosettaRNA在这一数据集上的优异表现进一步证明了其强大的泛化能力。

| 模型 | 中位数RMSD (Å) | 中位数INF_ALL | 中位数TM-score | 中位数lDDT |
| --- | --- | --- | --- | --- |
| **trRosettaRNA** | **9.9** | **0.82** | **0.49** | **0.66** |
| **NuFold** | 12.5 | 0.71 | 0.38 | 0.58 |
| **RNAComposer** | 11.2 | 0.76 | 0.43 | 0.62 |
| **FARFAR2** | 11.8 | 0.74 | 0.41 | 0.60 |
| **IsRNA2** | 12.1 | 0.73 | 0.40 | 0.59 |
| **SimRNA** | 13.2 | 0.70 | 0.37 | 0.56 |
...
AlphaFold3在Custom和CASP RNA数据集上的INF_ALL指标上表现领先：

| 数据集 | 中位数RMSD (Å) | 中位数INF_ALL | 中位数TM-score |
| --- | --- | --- | --- |
| **Custom** | 15.2 | **0.80** | 0.26 |
| **RNA-Puzzles** | 3.8 | 0.75 | 0.46 |
| **CASP RNA** | 10.5 | **0.85** | 0.47 |

### DRfold的性能

对于Custom数据集，DRfold在TM-score指标上表现领先：

| 数据集 | 中位数RMSD (Å) | 中位数INF_ALL | 中位数TM-score |
| --- | --- | --- | --- |
| **Custom** | 16.1 | 0.73 | **0.27** |
| **RNA-Puzzles** | 4.5 | 0.68 | 0.41 |
| **CASP RNA** | 11.8 | 0.72 | 0.39 |
...
Pearson相关系数分析显示，不同模型对假阳性碱基对的敏感性存在差异：

| 模型 | INF_ALL与 $p_{\mathrm{TP}}$ 相关系数 | INF_ALL与 $p_{\mathrm{FP}}$ 相关系数 |
| --- | --- | --- |
| **RNAComposer** | +0.75 | **-0.82** |
| **FARFAR2** | +0.72 | **-0.78** |
| **IsRNA2** | +0.71 | **-0.76** |
| **trRosettaRNA** | +0.68 | -0.65 |
| **NuFold** | +0.69 | -0.63 |
| **SimRNA** | +0.65 | -0.61 |

可以看出，**RNAComposer、FARFAR2和IsRNA2**对假阳性碱基对的敏感性更高（相关系数绝对值 $\ge 0.7$），这表明基于模板和从头方法更容易受到假阳性碱基对的影响。
