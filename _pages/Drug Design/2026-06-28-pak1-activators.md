...
graph TB
    ...
    subgraph R3["结果3：PAK1-A1/A2的发现"]
        C1["基于PAL-MS精炼位点第二次虚拟筛选"]
        C2["发现PAK1-A1=Eltrombopag"]
        C3["PAK1-A1：FDA已批准TPO激动剂"]
        C4["PAK1-A2：溶解度高、适合慢性给药"]
        C5["E99K模型用PAK1-A2（成药性更优）"]
        C1 --> C2 --> C3 --> C4 --> C5
    end
...
>
> - **第一**：建模方式是**人工融合蛋白**。AlphaFold2训练的是天然蛋白折叠，预测的只是这条人为序列最稳定的折叠方式，**不代表真实的PAP-PAK1结合模式。小编个人认为这是错误的**，最起码应该用multimer！
...
2. **激活机制的直接证据（图S1E）**：
   ...
   3. Cdc42本应该对孤立的激酶结构域提升效果有限（看图感觉不是的，小编认为存疑。。）？
