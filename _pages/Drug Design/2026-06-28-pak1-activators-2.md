...
- **自由能计算**：使用MM/GBSA方法（Molecular Mechanics with Generalized Born and Surface Area solvation），从400 ns轨迹提取结构，运行30个重复的5 ns平衡轨迹，用AmberTools的MMPBSA.py程序计算
...
- **图4E-F**：SPR传感器图（三次独立实验）——PAK1-A1（E）和NVS-PAK1-1（F）以浓度依赖方式结合全长PAK1，**稳态表观$K_D$分别在约1-4 μM和0.2-0.8 μM的低微摩尔范围**，与激酶活性EC$_{50}$/IC$_{50}$值一致
...
- **图4H/J：点突变（性质相反的氨基酸）验证关键残基**。
  - PAL-MS识别的8个残基中，Y131K/Y142K/E315A三个突变体在CHO细胞中无法表达或致死（提示对PAK1基本功能关键）；
...
**整体结论**：四组证据从静态结构（接触图）、活性动力学、竞争结合、动态暴露（HDX-MS）四个角度共同回答”PAK1-A1怎么把自抑制撬开”——结果是”推开KIS而非锁住DFG”的差异化姿态，正是Piano-Finger-Like机制的实验支撑
...
- **图5A-B：接触图揭示KIS被推开**
  - **接触图分析方法**：基于MD模拟轨迹计算蛋白内各结构域间的原子距离，用颜色/线条表示接触强度。apo态全长PAK1（A）中激酶抑制结构域紧贴蛋白骨架（密集接触）；PAK1-A1结合后（B），KIS区域的接触模式改变，远离激酶结构域——**自抑制界面被破坏**
  - **Piano-Finger-Like机制揭示**：PAK1-A1在DEK位点中通过疏水相互作用与Lys141、Phe408结合，同时与Glu315、Arg388、Asp407形成氢键，结合在激酶抑制片段原本占据的位置（距DEK位点约1.6 Å），从而**将激酶抑制结构域从催化结构域上推开**；而NVS-PAK1-1与DFG motif形成约1 Å的紧密相互作用，**稳定了DFG-out构象**。这就是论文提出的”**Piano-Finger-Like**”机制：激活剂和抑制剂虽然结合在重叠的DEK位点上，但采用了不同的结合姿态——PAK1-A1用**”推开”**姿态破坏自抑制，而NVS-PAK1-1用**”锁住”**姿态稳定DFG-out
- **图5C-D：活性动力学显示推开=激活**
...
- **图5E：竞争性结合显示调控元件重叠但不同**。PAK1-A1加到NVS-PAK1-1预处理过的PAK1中，先保持抑制后活性恢复；反之先PAK1-A1再加NVS-PAK1-1动力学被减慢——**这种”先入为主”行为直接支持Piano-Finger机制**：激活剂和抑制剂调节的是重叠但不同的调控元件
...
> 
> **Piano-Finger-Like机制的核心**：激活剂和抑制剂虽然结合在重叠的DEK位点上，但采用了不同的结合姿态——PAK1-A1用**”推开”**姿态破坏自抑制（距DEK位点约1.6 Å），而NVS-PAK1-1用**”锁住”**姿态稳定DFG-out（约1 Å紧密相互作用）。这种精细差异本身就体现了变构调节的核心原理：**同一位点的微小差异就能决定激活还是抑制**。
...
体外实验表明，**Cpd1和Cpd2能够浓度依赖性地增强PKA的活性**，Cpd1（conivaptan）EC$_{50}$约为**1.53 μM**，Cpd2（bemcentinib）EC$_{50}$约为**6.43 μM**。**当去除PKA的调节亚基后，激活作用消失**，表明这些化合物通过破坏自抑制相互作用来激活PKA。FRET生物传感器实验显示，Cpd1和Cpd2处理能够诱导PKAR4传感器的FRET比值时间依赖性增加。

![si4](pak1-allosteric-activators/si/figS4.png)

**补充图S4：小分子调节PAK1全局构象变化（图4补充）**
- **图S4A-B**：PAK1-A1（A）和NVS-PAK1-1（B）到结合位点中心的预测距离，每条轨迹代表一次独立MD模拟，两个体系均有**3次重复**——说明两类配体都能稳定占据DEK位点，但接触重心不同
...
- **图S5C-D**：MM/GBSA自由能分解分别识别PAK1-A1（C）和NVS-PAK1-1（D）结合全长PAK1时的关键残基，灰色标注表示两类配体共享的接触残基，用来解释两者为什么占据重叠位点却产生相反功能
...
- **口服生物利用度的限制**：目前发现的PAK1激活剂（如PAK1-A1和PAK1-A2）主要通过注射给药，口服生物利用度较低。这限制了其临床应用的便利性，未来需要通过结构优化来改善药代动力学特性。Eltrombopag（PAK1-A1）在PAK1上的EC$_{50}$远高于其作为TPO激动剂的药代窗口（nM级），重新定位需要解决选择性（PAK1 vs TPO/c-Mpl）和剂量学问题，但论文未深入讨论
