...
- **MD模拟预测PAK1-A1结合残基Lys141、Glu315、Arg388、Phe408**（DEK位点疏水相互作用），**与PAL-MS位点一致**——计算层面的内部自洽
  - **PAK1-A1没有X-ray/cryo-EM结构**：浸泡未检测到配体密度（结果4图注）——X-ray这条路走不通
...
- **图5G：HDX-MS动态暴露直接“看到”KIS被置换**。
  - 图中在AlphaFold2预测的PAK1结构上**高亮显示氘摄取变化显著的肽段**：**橙色表示ΔD bound-apo < −0.2（氘摄取降低，构象变紧凑/保护增加）**，**紫色表示ΔD bound-apo > 0.2（氘摄取增加，构象灵活性增加/被置换）**
