...
- **图S3H**：PAK1-A1（橙色）和NVS-PAK1-1（绿色）在重叠结合位点的细节对比，红色标注为共享残基。抑制剂更朝里一点
...
| **PAK1-A1** | Eltrombopag（FDA已批准TPO受体激动剂） | $\mathrm{EC}_{50} = 1.6 \pm 0.23~\mu\mathrm{M}$ | 约3-5倍 | 选择性激活PAK1，对PAK2/3无激活 | 安全性数据现成，可快速转化 |
...
- **图4G/I：FRET响应——上升=变近、下降=变远**。图4G（PAK1-A1）：FRET比值显著上升，**最大幅度Δ13.8%** → YFP和CFP距离**变近** → N端C端靠近。图4I（NVS-PAK1-1）：FRET比值下降 → YFP和CFP距离**变远** → N端C端远离、构象更开放，振幅在500 nM至20 μM浓度范围内不变但动力学呈剂量依赖。同一个传感器**对两类配体产生方向相反的响应**
