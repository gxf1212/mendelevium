...
- 在关键的概念、结论或punchline处，请使用 `**加粗**` 来突出重点，但不要包含最后一个句号，**要放在最后一个句号之前，否则渲染不成功。**也不要包裹非常规内容，如公式不要在加粗里面，**“黑箱”**这种带引号的，**贝叶斯优化（Bayesian Optimization）**这种带括号的，应该改成“**黑箱**”、**贝叶斯优化**（Bayesian Optimization），或者不加粗。**24%**这种都改成**24**%，否则报错。就包含正常汉字、数字等就行。加粗不要太短，**除**催化残基Lys83和Tyr51这种一个字的应该避免；**700倍以上**这种是可以的。
- 正文（引用格式、代码之类的、frontmatter和mermaid图之外）尽量用中文标点。括号逗号冒号引号什么的都考虑，参考tools底下的脚本。
...
- Markdown格式：列表这些的两个item之间不要有个空行，包括有序列表，其他比如上下也要适当多空行。图片、段落、heading的上下都应该有空行。只是修复列表内部，不删掉Markdown正常的空行。参考tools/fix_format.sh。如
  ...
  - 第三项
  ```
  - 列表不要滥用，是为了层级更清晰，列表太多反而影响易读性。每个item不要太短，而应该是一段话辅以适当加粗更合适。为了列举数据、参数之类的，表格更清晰。**但也不能完全没列表**，要符合“一条item大概几十个字，一般一段话超过200个汉字才拆成列表或多段”的要求。例子：
...
- **公式规范**:
    ...
    - 对于带单位的物理量，请使用正体表示单位，例如 `$\Delta\Delta G = -3.69 \mathrm{kcal/mol}$`，或者将单位写在公式外部。kJ·mol−1·nm−2这种涉及上标的的格式，用正体公式$2000\,\mathrm{kJ\cdot  mol^{-1}\cdot nm^{-2}}$这种，或sup也行啊，尽量不用那个小的字符，不要用mol⁻¹·nm⁻²这种。涉及上下标的都改，不涉及的、普通字体就规范的可以不改。
    ...
    - 行间公式用\dfrac而不是\frac
    - 连续的多个居中行间公式，方程组之类的，不要多个$$环境，而是\\分隔每一行。如有必要也可以用\begin{aligned}来对齐等号
    - $R(t)\propto t^{1/3}$，$B_2 \approx 0$ 这种过于短的公式就不用行间了，行内就行
...
- 结果和方法详述等地方：避免大段话，尽量改成列表，当然列表也不要太细碎，一条item大概几十个字。一般一段话超过200个汉字才拆，一般一段话一个要点。流程类的还是可以拆。并列的几个点还是可以用列表的。如**RA95.5-8F实现构象选择机制**：无论有无抑制剂，L1和L6位置基本不变……这种要用的，只要每个单点足够长。每个列表的item都长一点，大概100字+，除了公式解释变量的那些，所以该合并就合并
