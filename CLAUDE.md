...
在关键的概念、结论或punchline处，请使用 `**加粗**` 来突出重点，但不要包含最后一个句号，**要放在最后一个句号之前，否则渲染不成功。**也不要包裹非常规内容，如公式，**“黑箱”**这种带引号的，**贝叶斯优化（Bayesian Optimization）**这种带括号的，应该改成“**黑箱**”、**贝叶斯优化**（Bayesian Optimization），或者不加粗。**24%**这种都改成**24**%，否则报错。就包含正常汉字、数字等就行。正文尽量用中文标点。
...
- 正文部分的所有Figure和table必须出现，其中表格内容必须完整提取出来，但完整的可以放在附录或让我去截图，图我会自己粘进来，如果有![](fep_omega/fig3.png)这种，应该放在图注上方。
