...
- 在关键的概念、结论或punchline处，请使用 `**加粗**` 来突出重点，但不要包含最后一个句号，**要放在最后一个句号之前，否则渲染不成功。**也不要包裹非常规内容，如公式不要在加粗里面，`代码`也是，$2 \le `p_hill` $< 3$，也不要嵌套。。。。**“黑箱”**这种带引号的，**贝叶斯优化（Bayesian Optimization）**这种带括号的，应该改成“**黑箱**”、**贝叶斯优化**（Bayesian Optimization），或者不加粗。**24%**这种都改成**24**%，否则报错。就包含正常汉字、数字等就行。加粗不要太短，**除**催化残基Lys83和Tyr51这种一个字的应该避免；**700倍以上**这种是可以的。连字符之类的加粗没问题。一大片、好多段，里面不能一个加粗都没有，尽量每一段都有点加粗。
