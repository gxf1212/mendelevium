...
- 中文与拉丁字母、数字、英文单词之间加一个半角空格，例如"训练 14 个模型""使用 ROC-AUC 指标评估""在 GROMACS 中跑模拟"。
- 中文与中文标点（，。：；、""''（）《》）之间不空格。
- 行内数学公式 `$...$` 与相邻中文之间保留一个半角空格，例如"自由能差 $\Delta G$ 约为 -10 kcal/mol"。
- 化学式 `\ce{}` 是特例：紧贴其前后中文、无空格，例如"$\ce{Ca^{2+}}$ 离子"不空格（与上面数学公式规则相反，需牢记）。
- 数字序号与中文之间按列表规则处理：有序列表"2.随机森林"的"2."后不要空格（否则 mermaid/渲染异常）；普通行文中"第 3 章"的数字与中文之间可加空格。
- 英文单词之间、英文标点后的空格按英文习惯，不受上述约束。
